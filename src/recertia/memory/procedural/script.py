"""Derive a topological shell script from a ``SkillVersion``'s steps.

Used by scripted solve paths and M1-era tests. Non-shell tools become ``true`` no-op
markers here; production apply/adapt uses ``SkillApplicator`` wave execution instead.
"""
...
def script_from_skill(version: SkillVersion) -> list[str]:
    """Topological shell-step script; non-shell tools become no-op markers."""
