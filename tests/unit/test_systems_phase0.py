...
from contracts.policy import Policy, StateManagement
...
def test_tool_result_cache_hit_miss_and_write_skip(tmp_path: Path) -> None:
...
def test_offload_round_trip_hash_stable(tmp_path: Path) -> None:
    ...
    assert handle.sha256 != handle2.sha256 or handle.original_bytes == handle2.original_bytes
